...
open-source systems \pkg{Moodle} -- developed by \cite{exams:Moodle:2013} 
...
and with the recent fork \pkg{OpenOLAT} being developed by \cite{exams:OpenOLAT:2013}
and a support community. A popular proprietary
learning management system is \pkg{Blackboard} developed by \citep{exams:Blackboard:2010}.
...
for example employed in the \pkg{ONYX} testsuite \citep{exams:BPS:2013}
...
assessments\footnote{Currently, \pkg{OLAT} and \pkg{OpenOLAT} do not differ
with respect to their specification of exams. Hence, essentially
all discussion of \pkg{OLAT} in this manuscript applies to both \pkg{OLAT}
and \pkg{OpenOLAT}.}.

The new version of the \pkg{exams} package for the \proglang{R} system for 
statistical computing \citep{exams:R:2013} is available from the
...
\citep[see e.g.,][]{exams:Knuth:1992,exams:deLeeuw:2001,exams:Leisch+Rossini:2003,exams:Kuhn:2013}
...
\begin{itemize}
  ...
  \item Exams can be generated by randomly drawing different versions of exercises from a
    ...
    rendered into various formats including PDF, HTML, \pkg{Moodle} XML, or QTI~1.2
    (for \pkg{OLAT}/\pkg{OpenOLAT}).
...
the design underyling the toolbox for the new infrastructure. This section -- as well
...
\code{encoding}   & Character specifying the encoding to be used (in version~2 interfaces only). \\\hline
\end{tabular}
...
\code{n = 5} random exams and produce one exam and one solution PDF for each. The \code{template}
...
provided by \cite{exams:Gruen+Zeileis:2009}, also provided in an updated version as
\code{vignette("exams", package = "exams")}. The latter also discusses some small changes
and enhancements made for \code{exams()} (and \code{exams2pdf()}, respectively).
...
The function \code{exams2pdf()} is mainly a proof-of-concept reimplementation of \code{exams()}
...
of customization of the PDF output because this is discussed in \cite{exams:Gruen+Zeileis:2009}
...
\citep{exams:RStudio:2013}, versions prior to~0.97.133, the \code{"browser"} option is set
...
Here, \pkg{Iceweasel} is used -- Debian Linux's rebranding of the \pkg{Firefox} browser which has native MathML support.
...
dir(mydir)
@
The directory then contains the file `\code{demo.R}' which can be opened in
any editor for \proglang{R}~scripts. The script illustrates how to create
various kinds of output using the \code{exams2html()},\linebreak \code{exams2pdf()},
...
directory but the `\code{demo.R}' script only employs one example for each
exercise type, i.e., \code{num}, \code{schoice}, \code{mchoice}, \code{cloze}, and \code{string}.
To restrict this set of exercises, the \code{type} argument of \code{exams_skeleton()}
can be used (e.g., \code{type = c("num", "schoice")}). In any case,
it should be easy to modify the `\code{demo.R}' script, omitting or adding
...
\code{encoding = "UTF-8"}. This modifies the \code{demo.R} script as well
...
\verb|\extolerance{}| & Tolerance for \code{num} solutions or a vector of tolerances (expanded if
...
is employed in \code{boxhist} (with more flexible formatting specifically
for \pkg{Moodle} in \code{boxhist2}). See also Table~\ref{tab:exercises} in the
...
convenient options: e.g., using the mathematical markup language MathML \citep{exams:W3C+MathML:2010,exams:Wiki+MathML:2013}
...
image in a \code{system()} call to \pkg{ImageMagick}'s \code{convert} function \citep{exams:ImageMagick:2013}.
...
\code{mchoice} XML driver would be \code{make_question_moodle23(solution = FALSE, shuffle = TRUE)}.
...
official support for this type of exercises, although they actually work correctly.\footnote{The
only aspect that does not work for \texttt{<response\_num>} exercises is the display of
correct solutions in the final results page of an exam.}
Hence, optionally \code{exams2qti12()} offers a
workaround: If the \code{digits} argument is some finite value (e.g., \code{digits = 2}),
...
if the string \code{111.10} with \code{digits = 2} is used). 
...
\code{exams2lops()} or \code{exams2pdf()} interfaces are employed.
...
\code{dist2}       & \code{cloze}   & Extended cloze version of the numeric \code{dist}.\\ \hline
...
penalized (and thus not different from unanswered exercises) or
