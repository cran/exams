...
\Abstract{ 
  This first introduction to the \proglang{R} package \pkg{exams} is a (slightly)
  ...
  files from exercises in Sweave format. Meanwhile, \pkg{exams} has been
  considerably extended by \cite{exams:Zeileis+Umlauf+Leisch:2012} to
  also produce HTML output or e-learning exams for \pkg{Moodle}, \pkg{OLAT}, etc.
...
Wien (WU Wien, \url{http://www.wu.ac.at/}), Vienna, Austria, is
...
\begin{itemize}
  ...
  \item \verb|\extol{}|: optional tolerance limit(s). For numeric solutions
	...
	different tolerances can be supplied for falling below or exceeding the
	true value via \verb|\extol{|\emph{lower}\verb|}{|\emph{upper}\verb|}|.
        \update{Note that asymmetric tolerance intervals are only supported in
	\texttt{exams} but not in \texttt{exams2pdf} or any of the other new
	interface functions.}
\end{itemize}
...
object with the meta-information invisibly. \update{The function \texttt{exams2pdf()}
is a more flexible rewritten version of \texttt{exams()} and be used almost
interchangeably. Its use is recommended for new projects.}
%
...
@ 
<<tstat-non-interacive, echo=FALSE, eval=TRUE>>=
set.seed(1090)
tdir <- tempdir()
...
to the solution string). Note that this implementation
...
decimal mark for numeric results is possible. \update{The fixed-length implementation
for \texttt{mchoice} has been relaxed in \texttt{exams2pdf()}. This
uses only one required argument and an arbitrary number of optional arguments
instead.}
...
provided in \pkg{exams}: \code{oexam.tex} and \code{osolution.tex}.
\update{Previously, these were named \texttt{exam.tex} and \texttt{solution.tex}.
However, due to the change in the \texttt{exmchoice} command mentioned above,
the original template files are not fully compatible with \texttt{exams2pdf()} and were
therefore renamed.}
\code{oexam.tex} can be used
...
for the exercises, obviously). \code{osolution.tex} produces PDF files containing a
cover sheet similar to \code{oexam.tex} but with the correct answers
...
list.files(odir)
@ 
and can now be easily inspected by the reader. In addition to the $2
\cdot 5$ PDF files, there is a file \code{metainfo.rda} containing the
\code{save()}d ``\code{\Sexpr{class(sol)}}'' object which is also
stored in object \code{sol} above. When evaluating an exam, this can
be conveniently \code{load()}ed and individual results can be printed:
<<>>=
print(sol, 1)
print(sol, "exam5")
@
