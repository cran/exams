...
switch(type,
"labor" = {
  question <- paste("What is the amount of the input factor \\\\\\textit{labor} in this minimum?")
  solution <- paste("Given the target output, the optimal amount of the input factor \\\\\\emph{labor} is $L = ", fmt(L), "$.", sep = "")
  sol <- L
  com <- "%"
},
"capital" = {
  question <- paste("What is the amount of the input factor \\\\\\textit{capital} in this minimum?")
  solution <- paste("Given the target output, the optimal amount of the input factor \\\\\\emph{capital} is $K = ", fmt(K), "$.", sep = "")
  sol <- K
  com <- "%"
},
"costs" = {
  question <- paste("How high are in this case the minimal costs?")
  solution <- paste("Given the target output, the minimal costs are $", fmt(cost), "$.", sep = "")
  sol <- cost
  com <- ""
})
...
The price for one unit of \textit{capital} is $p_K = \Sexpr{p_K}$ 
and the price for one unit of \textit{labor} is $p_L = \Sexpr{p_L}$.
...
\min_{K,L} C(K,L) &=& p_KK + p_LL\\
 &=& \Sexpr{p_K}K + \Sexpr{p_L}L\\
\mbox{unter der NB:} &&  F(K,L) = Q \\
