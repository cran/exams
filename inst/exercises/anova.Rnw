...
myanova_print[, 3] <- format(round(myanova[, 3]))
...
if(runif(1) > 0.5) {
  ...
  explanations[1] <- paste("The test statistic is $F = ", format(round(myanova[2,5], digits = 3), nsmall = 3),
...
solutions[3] <- r2 > r2a
explanations[3] <- paste("The fraction of explained variance is $", format(round(r2, digits = 3), nsmall = 3), 
...
solutions[5] <- r2 < r2b
explanations[5] <- paste("The fraction of explained variance is $", format(round(r2, digits = 3), nsmall = 3), 
...
\begin{question}
  ...
  respondents in Figure~\ref{fig:ch05-anova1}.
...
@
\caption{\label{fig:ch05-anova1} Parallel boxplots}
\end{center}
\end{figure}

To analyse the influence of occupation on the evaluation of the
...
\begin{itemize}
  \item[(a)] \Sexpr{mchoice2text(solutions[1])}: \Sexpr{explanations[1]}
  \item[(b)] \Sexpr{mchoice2text(solutions[2])}: \Sexpr{explanations[2]}
  \item[(c)] \Sexpr{mchoice2text(solutions[3])}: \Sexpr{explanations[3]}
  \item[(d)] \Sexpr{mchoice2text(solutions[4])}: \Sexpr{explanations[4]}
  \item[(e)] \Sexpr{mchoice2text(solutions[5])}: \Sexpr{explanations[5]}
\end{itemize}
\end{solution}
