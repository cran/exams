...
(for online learning and training), developed by \cite{exams:OLAT:2012},\footnote{
Note that the \pkg{OLAT} project was forked recently and now some of the
core contributors develop \pkg{OpenOLAT} \citep{exams:OpenOLAT:2012}.
Essentially all discussion of \pkg{OLAT} in this manuscript also applies to \pkg{OpenOLAT}
...
While \pkg{Moodle} relies on its own \pkg{Moodle} XML format, \pkg{OLAT} and \pkg{Blackboard}
...
\code{dir}        & Path to output directory. Default: Single PDF or HTML files are shown directly
		    ...
		    with \code{n = 1}). In all other cases the current working directory is used.\\ 
...
\code{template}   & Character specifying the (base) names of a {\LaTeX}, HTML, or XML file
		    ...
		    A function-specific template provided within the \pkg{exams} installation.\\ \hline
\end{tabular}
...
Here, \pkg{Iceweasel} is used -- Debian's rebranding of the \pkg{Firefox} browser which has native MathML support.
...
\proglang{R}~package \pkg{tth} \citep{exams:Hutchinson+Leisch+Zeileis:2012} that makes
...
dir(odir)
@
This XML file \code{moodlequiz.xml} can be easily imported into a \pkg{Moodle} quiz and then further
customized: First, the XML file is imported into the question bank in \pkg{Moodle}. Then,
all replications of each exercise can be added as ``random'' questions into a quiz
(and potentially further customized). Figure~\ref{fig:exams2moodle-boxplots} shows the first random draw
of the \code{boxplots} exercise in the resulting \pkg{Moodle} quiz (again rendered
by a \pkg{Firefox} browser).

\begin{figure}[t!]
\centering
\includegraphics[width=0.9\textwidth]{screenshots/exams2moodle-boxplots.png}
...
\begin{figure}[t!]
\centering
\includegraphics[width=0.93\textwidth]{screenshots/exams2qti12-boxplots.png}
...
generate them with extra empty columns. The \code{cholesky} exercise template has
...
not for other files (such as data sets etc.) and can optionally also be disabled if
desired.
...
the appearance of exam/exercises, see \code{?exams2qti12} for details. Also, the
underlying XML template could be adapted and extended.
...
then \emph{written} to suitable output file formats. Different customizable
...
\verb|\exsolution{}|  & Correct solution (required). It must contain a numeric solution for \code{num},
			...
			be specified, e.g., \verb+\exsolution{1.23|001|median}+. \\
\verb|\extolerance{}| & Tolerance limits (of length~1 or~2) for \code{num} solutions. If unspecified
                        the tolerance is 0. For length~2 (not supported in \pkg{Moodle}), the lower and upper tolerance can be
			specified separately.\\
\verb|\exclozetype{}| & List of types for the elements of a \code{cloze} exercise, e.g.,
                        \verb+\exclozetype{num|mchoice|string}+ for the example above. \\ 
\verb|\expoints{}|    & Points for correct solution. Default is 1.\\
...
%% \verb|\exsingle{}|    & Logical indicating whether radio buttons should be used in \pkg{Moodle}. \\
\hline
\end{tabular}
...
\begin{itemize}
  ...
  \item \code{cloze} for a combination of questions/statements with \code{num},
    ...
    has either a numeric, short text, or single/multiple-choice answer, e.g., \verb+\exsolution{1.23|001|median}+.
...
can be provided. The \code{{answerlist}} environment can either be written as usual ``by hand''
...
is employed in \code{boxhist}. See also Table~\ref{tab:exercises} in the
...
convenient options: e.g., using the mathematical markup language MathML \citep{exams:W3C+MathML:2010,exams:Wiki+MathML:2012}
...
long been available. More recently, other major browsers like Google \pkg{Chrome}, \pkg{Opera}, or \pkg{Safari}
also added support for MathML \citep[see][Section~1.2]{exams:Vismor:2012}. Furthermore, \pkg{MathJax}
\citep{exams:MathJax:2012}, an open-source \proglang{JavaScript} engine, can be used to render
MathML (or {\LaTeX}) formulas on a server rather than in the local browser.
...
former two come from the \proglang{R} package \pkg{tth} \citep{exams:Hutchinson+Leisch+Zeileis:2012}
...
image in a \code{system()} call to \pkg{ImageMagick}'s \code{convert} function \citep{exams:ImageMagick:2012}.
...
\begin{itemize}
  ...
  \item \code{ttm} is preferable if there is some moderately complicated mathematical notation
  ...
  \item \code{tex2image} is the ``last resort'' if neither of the two previous approaches work.
    For example, if more complex {\LaTeX} commands/packages need to be used which are not
    supported by \code{tth}/\code{ttm}.
\end{itemize}
...
\caption{\label{fig:plain.html} Default HTML \code{template} file \texttt{plain.html} employed in
  \texttt{make\_exams\_write\_html()}. Elements marked by \texttt{\#\#} are being replaced in each
...
\begin{itemize}
  ...
  \item For each question a title text is included (in suitable XML tags),
    where \code{iname}/\code{stitle}/\code{testid} can be used for the fine-tunging.
...
By default, the package has a flexible generator \code{make_question_moodle23()}
...
mchoice XML driver would be \code{make_question_moodle23(solution = FALSE, shuffle = TRUE)}.
Thus, while only a single generator function is available, one can easily set
different argument lists for numeric or multiple-choice exercises etc. See
the corresponding manual page for an overview of the fine-control options.

The approach take in \code{exams2qti12()} is essentially analogous to that
...
One detail of the QTI~1.2 interfaces should be briefly explained: Although the QTI~1.2
XML standard supports numeric exercises/items through its \verb|<response_num>| tag, this is
not fully implemented in all QTI-based learning management systems. Namely, \pkg{OLAT} has no
official support for this type of exercises. Hence, \code{exams2qti12()} by default
uses the following workaround: If the \code{digits} argument is some finite value (default: \code{2}),
the correct numeric solution is formatted to a character string with \code{digits} decimal
places. Then, the correct solution is entered as a string solution (\verb|<response_str>|)
which means that answers entered in the system will only be recognized as correct if they
exactly match the correct string (e.g., \code{111.1} would not be recognized as correct
if the string \code{111.10} with \code{digits = 2} is used). Alternatively, one can set
\code{num = list(digits = NULL)} in \code{exams2qti12()} so that \verb|<response_num>|
will be used in the XML code. This has the inteded effect that the solution is recognized
as numeric and the corresponding \code{tolerance} is employed, however it also has the
uninteded side effect that correct solutions are not displayed in the final results page of
the exam. Hence, we have set \code{digits = 2} by default but we hope that future releases
of \pkg{OLAT} are improved to also fully support \code{digits = NULL}.
...
\end{itemize}
Although, the \pkg{exams} package can also generate PDF exams directly, an interface to this exam
server is desirable because it can handle the OCR automatically and the students can easily log into
the exam server to see their personal results and inspect their exam scan.
...
\url{http://R-Forge.R-project.org/forum/?group_id=1337}.
...
\code{exams2lops()} interface is employed.
...
such as \pkg{ONYX}. This may also be one potential route for support
of \pkg{Blackboard} which we have not yet been able to investigate
due to lack of access to the proprietary \pkg{Blackboard} system. An
alternative could be a direct adaptation of the \pkg{Blackboard}
flavor of the QTI~1.2 XML format.

Furthermore, users may be interested in extensions/adaptions of existing
...
\code{confint}     & \code{num}     & Confidence interval for one-sample mean, i.e., result of lenght two (for version~1 interface).\\
...
\code{confint2}    & \code{cloze}   & Cloze version (for all version~2 interfaces) constructed from the numeric \code{confint}.\\
