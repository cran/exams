...
respectively. They always produce just a single XML or ZIP file
which can be easily uploaded into \pkg{Moodle} and \pkg{OLAT}, respectively.
...
\code{confint}     & \code{num}     & Confidence interval for one-sample mean, i.e., result of lenght two.\\
...
\code{anova}       & \code{mchoice} & Interpretation of \code{anova()} table and corresponding parallel boxplots.\\
...
\code{boxhist}     & \code{cloze}   & Based on randomly generated data as (exercise-specific) \code{.csv} files
				      ...
				      and histogram have to be made (\code{mchoice}). \\ \hline
\end{tabular}
